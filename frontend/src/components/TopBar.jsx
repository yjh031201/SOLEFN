    ...
    <header className="top-header">
      ...
      <div className="header-right">
        <div className="top-links">
          <Link to="/customer" style={{ textDecoration: 'none', color: 'inherit' }}>고객센터</Link>
          <Link to="/mypage" style={{ textDecoration: 'none', color: 'inherit' }}>마이페이지</Link>
          {isLogin ? (
            <span onClick={() => setIsLogin(false)} style={{ cursor: 'pointer' }}>로그아웃</span>
          ) : (
            <span onClick={() => setIsLogin(true)} style={{ cursor: 'pointer' }}>로그인</span>
          )}
        </div>
